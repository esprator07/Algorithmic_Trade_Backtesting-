...
def find_exit_and_manage_tsl(symbol, start_candle_index, df_4h, entry_time, initial_peak, initial_tsl, position_type, TRAILING_STOP_PERCENT):
    ...
    # Girişin yapıldığı 4s mumundan başla
    for i in range(start_candle_index, len(df_4h)):
        ...
        # Taramanın pozisyon açıldıktan sonra başlamasını sağlayan filtre
        ...
        # İlk 4s mumundan sonraki taramalar için entry_time'ı sıfırla ki tüm mumu tarasın
...
def run_backtest(BACKTEST_DAYS, RSI_PERIOD, RSI_OVERSOLD, RSI_OVERBOUGHT, TRAILING_STOP_PERCENT):
    ...
    for symbol in COIN_LIST:
        print(f"\n{'='*30}\n▶️ {symbol} için TSL Backtest Başlatılıyor...\n{'='*30}")
        ...
        while i < len(df_4h):
            ...
            if signal_candidate:
                ...
                if entry_price and entry_time:
                    ...
                    print(f"🚀 YENİ POZİSYON AÇILDI: {symbol} | TÜR: {position_type} | GİRİŞ: {entry_price:.5f} | İLK TSL: {trailing_stop_price:.5f} | Tarih: {entry_time}")
                    ...
                    i = exit_candle_index # Ana döngüyü pozisyonun kapandığı mumun sonrasına atlat
                else:
    ...
    report_lines.append("🏆 SADECE TRAILING STOP LOSS BACKTEST SONUÇLARI 🏆")
    report_lines.append(separator)
...
def start_optimization():
    ...
    print(f"Trailing Stop Optimizasyonu Başlatılıyor... Toplam {total_combinations} farklı parametre kombinasyonu test edilecek.")
