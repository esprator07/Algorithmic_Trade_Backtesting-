...
# Pozisyon yönetimi için ana fonksiyon
def find_exit_and_manage_position(symbol, start_candle_index, df_4h, entry_time, tp_price, initial_peak, initial_tsl, position_type, TRAILING_STOP_PERCENT):
...
def run_backtest(BACKTEST_DAYS, RSI_PERIOD, RSI_OVERSOLD, RSI_OVERBOUGHT, TAKE_PROFIT_PERCENT, TRAILING_STOP_PERCENT):
    ...
    overall_results = {
        ...
        'total_commission_percent': 0.0, 'total_funding_fee_percent': 0.0,
        'tp_wins': 0, 'tsl_wins': 0, 'tsl_losses': 0,
        'tsl_win_pnl_sum': 0.0, 'tsl_loss_pnl_sum': 0.0, 'timeout_closes': 0
    }
    coin_performance = {}

    for symbol in COIN_LIST:
        print(f"\n{'='*30}\n▶️ {symbol} için TP+TSL Backtest Başlatılıyor...\n{'='*30}")
        ...
        while i < len(df_4h):
            ...
            if signal_candidate:
                ...
                if entry_price and entry_time:
                    ...
                    print(f"🚀 YENİ POZİSYON AÇILDI: {symbol} | TÜR: {position_type} | GİRİŞ: {entry_price:.5f} | TP: {tp_price:.5f} | İLK TSL: {trailing_stop_price:.5f} | Tarih: {entry_time}")
                    
                    outcome, exit_price, exit_time, exit_candle_index = find_exit_and_manage_position(symbol, i, df_4h, entry_time, tp_price, peak_price, trailing_stop_price, position_type, TRAILING_STOP_PERCENT)
                    
                    if outcome == 'TP':
                        gross_pnl_percent = TAKE_PROFIT_PERCENT
                    else:
                    ...
                    if outcome == 'TP':
                        ...
                        if net_pnl_percent > 0:
                            overall_results['tsl_wins'] += 1
                            overall_results['tsl_win_pnl_sum'] += net_pnl_percent
                            if position_type == 'LONG': overall_results['long_wins'] += 1
                            else: overall_results['short_wins'] += 1
                        else:
                            overall_results['tsl_losses'] += 1
                            overall_results['tsl_loss_pnl_sum'] += net_pnl_percent
                            if position_type == 'LONG': overall_results['long_losses'] += 1
                            else: overall_results['short_losses'] += 1
    ...
    report_lines.append("🏆 HİBRİT TP + TSL BACKTEST SONUÇLARI 🏆")
    report_lines.append(separator)
    ...
    report_lines.append(f"Trailing Stop Oranı: %{TRAILING_STOP_PERCENT * 100:.2f}")
    ...
    avg_tsl_win_pnl = (overall_results['tsl_win_pnl_sum'] / overall_results['tsl_wins'] * 100) if overall_results['tsl_wins'] > 0 else 0
    avg_tsl_loss_pnl = (overall_results['tsl_loss_pnl_sum'] / overall_results['tsl_losses'] * 100) if overall_results['tsl_losses'] > 0 else 0
    report_lines.append("--- Çıkış Stratejisi Dökümü ---")
    report_lines.append(f"Take Profit ile Kapanan: {overall_results['tp_wins']} İşlem")
    report_lines.append(f"TSL ile Kârlı Kapanan: {overall_results['tsl_wins']} İşlem (Ort. Kâr: {avg_tsl_win_pnl:+.2f}%)")
    report_lines.append(f"TSL ile Zararlı Kapanan: {overall_results['tsl_losses']} İşlem (Ort. Zarar: {avg_tsl_loss_pnl:.2f}%)")
    ...
    try:
        ...
        filename = (f"analizler/hybrid_tp{tp_str}_tsl{tsl_str}_{BACKTEST_DAYS}d_rsi{RSI_PERIOD}-{RSI_OVERSOLD}-{RSI_OVERBOUGHT}_pnl{pnl_str}.txt")
...
def start_optimization():
    ...
    tp_range = np.arange(0.02, 0.051, 0.01)
    tsl_range = np.arange(0.01, 0.021, 0.005)
    ...
    print(f"Hibrit TP+TSL Optimizasyonu Başlatılıyor... Toplam {total_combinations} farklı parametre kombinasyonu test edilecek.")
        ...
        for rsi_ob in RSI_OVERBOUGHT_LIST:
            ...
            for tp in tp_range:
                for tsl in tsl_range:
                    ...
                    print(f"Parametreler: RSI={RSI_PERIOD}/{rsi_os}/{rsi_ob} | TP=%{tp*100:.2f} | TSL=%{tsl*100:.2f}")
                    print(f"{'#'*60}\n")
